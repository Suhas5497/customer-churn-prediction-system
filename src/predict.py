...
from src.analytics import calculate_probability_percentile, enrich_scored_frame, segment_probability
from src.preprocessing import transform_features, validate_input_frame
...
def load_model_artifact(model_path: Path | str = MODEL_PATH) -> dict[str, Any]:
    ...
    if not isinstance(artifact, dict) or "model" not in artifact or "preprocessing" not in artifact:
        raise ValueError(
            "The saved model artifact is invalid. Run `python src/train_model.py` to rebuild it."
        )
    return artifact
...
def score_dataframe(
    ...
    transformed_input = transform_features(validated_input, preprocessing, strict=True)
    probabilities = pd.Series(model.predict_proba(transformed_input)[:, 1], dtype=float)

    scored = enrich_scored_frame(validated_input, probabilities, threshold=threshold)
    ...
    scored["probability_percentile"] = scored["predicted_probability"].apply(
        lambda probability: calculate_probability_percentile(probability, reference_scores)
    )
...
def get_local_explanation(
    ...
    resolved_artifact = _resolve_artifact(artifact, model_path)
    preprocessing = resolved_artifact["preprocessing"]
    validated_input = validate_input_frame(prepare_input_frame(input_data), preprocessing, strict=True)
    transformed_input = transform_features(validated_input, preprocessing, strict=True)
    ...
    explanation = pd.DataFrame(
        {
            ...
            "display_feature": [format_feature_name(feature) for feature in transformed_input.columns],
            "shap_value": contribution_values,
        }
    )
    explanation["impact"] = explanation["shap_value"].apply(
        lambda value: "Increase Churn Risk" if value >= 0 else "Reduce Churn Risk"
    )
    explanation["abs_shap_value"] = explanation["shap_value"].abs()

    return explanation.sort_values("abs_shap_value", ascending=False).head(top_n).reset_index(drop=True)
